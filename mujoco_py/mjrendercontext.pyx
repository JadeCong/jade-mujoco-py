...
cdef class MjRenderContext(object):
    ...
    def __cinit__(self):
        # maxgeom = 1000
        # mjv_makeScene(self._model_ptr, &self._scn, maxgeom) # from line(by JadeCong)
        ...
        # move all init to line 61
        pass  # to line(by JadeCong)
    ...
    def __init__(self, MjSim sim, bint offscreen=True, int device_id=-1, opengl_backend=None, quiet=False):
        ...
        mjv_defaultCamera(&self._cam)  # from line(by JadeCong)
        mjv_defaultPerturb(&self._pert)
        mjv_defaultOption(&self._vopt)
        mjr_defaultContext(&self._con)
        ...
        maxgeom = 1000
        mjv_makeScene(self._model_ptr, &self._scn, maxgeom)  # to line(by JadeCong)
